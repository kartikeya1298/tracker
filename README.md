...
behavior of baking `NEXT_PUBLIC_*` vars into the browser bundle at Docker build
time, which the Blueprint spec has no clean way to parameterize per-deploy.
`docker-compose.yml`'s existing `NEXT_PUBLIC_API_URL` build-arg flow is untouched
and still takes priority when set, so local Docker Compose usage is unaffected.
